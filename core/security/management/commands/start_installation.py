...
class Command(BaseCommand):
    ...
    def handle(self, *args, **options):
        dashboard = Dashboard.objects.create(
            name='LA VICTORIA',
            author='Universidad Agraria del Ecuador',
            footer_url='https://www.uagraria.edu.ec/',
            icon='fas fa-tractor',
        ...
        for module in Module.objects.filter().exclude(url__in=['/pos/client/update/profile/', '/pos/sale/client/']):
        ...
        user = User.objects.create(
            username='admin',
            names='FREIRE QUIÑONEZ VÍCTOR HUGO',
            email='freirehugo@gmail.com',
            is_active=True,
            is_superuser=True,
            is_staff=True
        )
        ...
        for module in Module.objects.filter(url__in=['/pos/client/update/profile/', '/pos/sale/client/', '/user/update/password/']):
            GroupModule.objects.create(module=module, group=group)
            for permission in module.permissions.all():
                group.permissions.add(permission)

    ...
    def post(self, request, *args, **kwargs):
        ...
        try:
            if action == 'add':
                ...
                data['valid'] = not Group.objects.filter(filters).exists() if filters.children else True
            else:
                data['error'] = 'No ha seleccionado ninguna opción'
        except Exception as e:
            data['error'] = str(e)
    ...
    def post(self, request, *args, **kwargs):
        ...
        try:
            if action == 'edit':
                ...
                if field == 'name':
                    filters &= Q(name__iexact=request.POST['name'])
                data['valid'] = not Group.objects.filter(filters).exclude(id=self.object.id).exists() if filters.children else True
            else:
                data['error'] = 'No ha seleccionado ninguna opción'
        except Exception as e:
            data['error'] = str(e)

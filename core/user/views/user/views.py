    ...
    def post(self, request, *args, **kwargs):
        ...
        try:
            if action == 'add':
                ...
                data['valid'] = not User.objects.filter(filters).exists() if filters.children else True
            else:
                data['error'] = 'No ha seleccionado ninguna opción'
        except Exception as e:
            data['error'] = str(e)
        ...
        try:
            if action == 'edit':
                ...
                if field == 'username':
                    ...
                    filters &= Q(email__iexact=request.POST['email'])
                data['valid'] = not User.objects.filter(filters).exclude(id=self.object.id).exists() if filters.children else True
            else:
                data['error'] = 'No ha seleccionado ninguna opción'
        except Exception as e:
            data['error'] = str(e)
        ...
        try:
            if action == 'edit':
                ...
                if field == 'username':
                    ...
                    filters &= Q(email__iexact=request.POST['email'])
                data['valid'] = not User.objects.filter(filters).exclude(id=request.user.id).exists() if filters.children else True
            else:
                data['error'] = 'No ha seleccionado ninguna opción'
        except Exception as e:
            data['error'] = str(e)

...
var production = {
    listTapeAssignments: function () {
        tblTapeAssignments = $('#tblTapeAssignments ').DataTable({
            ...
            columns: [
                ...
                {data: "tape"},
                {data: "latest_fruit_control_quantity"},
                {data: "quantity"},
                {data: "boxes"},
            ],
            columnDefs: [
                ...
                },
                {
                    ...
                    ordering: false,
                    render: function (data, type, row) {
                        return '<input type="text" class="form-control" autocomplete="off" name="boxes" value="' + row.boxes + '">';
                    }
                }
            ],
            ...
            initComplete: function (settings, json) {
                ...
                if (json.notification) {
                    return alert_sweetalert({
                        'type': 'info', 'message': json.notification, 'timer': 3000, 'callback': function () {
    ...
    getTapeAssignments: function () {
        if (tblTapeAssignments) return tblTapeAssignments.rows().data().toArray();
        return [];
    }
};
...
$(function () {
    ...
    $('#tblTapeAssignments  tbody')
        ...
        .on('change', 'input[name="boxes"]', function () {
            var tr = tblTapeAssignments.cell($(this).closest('td, li')).index();
            var data = tblTapeAssignments.row(tr.row).data();
            data.boxes = parseInt($(this).val());
        });
